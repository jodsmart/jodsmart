...
class EA_BB:
    ...
    def CalLot(self,total_orders):
        ...
        if total_orders>0:
            cal_lot=int(cal_lot*(self.obj_input.Multiply**total_orders)*100)/100
    ...
    def Open_Long_Control(self):
        ...
        if self.l_p_under + self.obj_input.Trail<= price:
            ...
            if self.buy_min_order is None:
                if self.obj_input.Manual==1 and self.flag ==1:
                    ticket=self.objClient.OrderSendBuy(self.symbol.Name, x,self.comment)
            ...
            elif self.obj_input.Max_order>self.buy_total_order:
                if float(self.buy_min_order._open_price) - self.obj_input.Order_diff > price:
                    ticket=self.objClient.OrderSendBuy(self.symbol.Name, x,self.comment)
        ...
        #     if float(self.buy_max_order._open_price)+self.obj_input.TP+self.obj_input.Trail*(2+len(self.buy_over_list))<price:
    ...
    def Close_Long_Control(self):
        ...
        if self.l_p_over==0:
            ...
            if o_price - self.obj_input.SL >= price or self.trend<=0:
                self.l_p_over=price
                msg = "Cut loss Flag Set :" + self.Name + " ID " + str(o_ticket)
                print(msg)
                print("Line msg : " + str(self.objClient.line.send_Line_msg(msg)))
            ...
            if obj_order is not None:
                if obj_order['_response_value']=="SUCCESS":
                    if float(obj_order['_profit'])>0:
                        ...
                        if self.l_p_over - o_price > self.obj_input.TP:
                            self.win+=1
                            self.failed=0
                    else:
    ...
    def Open_Short_Control(self):
        ...
        if self.s_p_over - self.obj_input.Trail >= price:
            ...
            if self.sell_max_order is None:
                if self.obj_input.Manual==1 and self.flag ==-1:
                    ticket=self.objClient.OrderSendSell(self.symbol.Name, x,self.comment)
            ...
            elif self.obj_input.Max_order>self.sell_total_order:
                if float(self.sell_max_order._open_price) + self.obj_input.Order_diff <= price:
                    ticket=self.objClient.OrderSendSell(self.symbol.Name, x,self.comment)
        ...
        #     if float(self.sell_min_order._open_price)-self.obj_input.TP-self.obj_input.Trail*(2+len(self.sell_under_list))>price:
    ...
    def Close_Short_Control(self):
        ...
        if self.sell_min_order is None:
            return False
        ...
        if self.s_p_under + self.obj_input.Trail<= price:
            ...
            if obj_order is not None:
                if obj_order['_response_value']=="SUCCESS":
                    if float(obj_order['_profit'])>0:
                        self.profit+=float(obj_order['_profit'])
                        if o_price - self.s_p_under > self.obj_input.TP:
                            self.win+=1
                            self.failed=0
    ...
    def Processing(self):
        #print("Processing")
        #Timer(t, self.Processing, args=(t,)).start()
        msg = ""      
        if self.order_count<self.obj_input.Order_loop:
            if (self.obj_input.long_only == 1):
                if self.Open_Long_Control():
                    ...
                    #self.flag=0
                    if self.failed==4:
                        msg=self.Name+" Failed :"+str(self.failed)
                        print(msg)
                        print("Line msg : " + str(self.objClient.line.send_Line_msg(msg)))
                    return

            if (self.obj_input.short_only == 1):
        ...
        if self.Close_Long_Control():
            self.l_p_under = 0
            if self.buy_total_order==0 and self.l_p_over!=0:
                self.order_count=0
                self.l_p_over=0
            return
        ...
        if self.Close_Short_Control():
            self.s_p_over = 0
            if self.sell_total_order==0 and self.s_p_under!=0:
                self.order_count=0
                self.s_p_under=0
            return
    ...
    def PrintStatus(self):
        ...
        print(tt + " : " + self.Name + 
              ...
              "),Failed(" + str(self.failed) + 
              "),Trend " + str(self.trend) +",Flag " + str(self.flag))
    ...
    def SetFlag(self):
        #print("SetFlag")
        price = self.symbol.Ask
        if self.ma[0][4]<=price<=self.ma[0][3] and self.ma[0][2]>self.ma[0][3] and self.flag==0 and self.trend==1  and self.buy_total_order==0:
            self.flag=1
            self.l_p_under=price
            msg=self.Name+" Set Flag Buy"
            print(msg)
            print("Line msg : " + str(self.objClient.line.send_Line_msg(msg)))
            self.PrintStatus()
            return
        ...
        price = self.symbol.Bid
        if self.ma[0][4]>=price>=self.ma[0][3] and self.ma[0][1]>self.ma[0][3] and self.flag==0 and self.trend==-1  and self.sell_total_order==0:
            self.flag=-1
            self.s_p_over=price
            msg=self.Name+" Set Flag Sell"
            print(msg)
            print("Line msg : " + str(self.objClient.line.send_Line_msg(msg)))
            self.PrintStatus()
            return
    ...
    # ------------------------------------------------------------------------------------------------------
    def __init__(self, symbol, obj_client,obj_input):
        self.Name=symbol.Name+str(obj_input.timeframe)
        print("------------------------------------EA BB "+self.Name+" Start-----------------------------------------------------")
        #--------------- Init Data -----------------------------------------------------------------------------------
        ...
        self.comment = "bb_ea"+str(obj_input.timeframe)
        self.db_name="bb_status"
        self.ExeFlag=False
