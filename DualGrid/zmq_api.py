...
from dualgrid import EA_DualGrid
...
class zmq_api(DWX_ZMQ_Strategy):
    ...
    def __init__(self,master_port,obj_input):
        ...
        self._symbols=obj_input.symbols
    ...
    def ReListTotalOrders(self,_data):
        ...
        if len(df_orders)>0:
            ...
            for i in range(len(df_orders)):
                get_ea = list(filter(lambda x: x.magic == df_orders.iloc[i]._magic and x.symbol.Name==df_orders.iloc[i]._symbol, self.ea))
                if len(get_ea) > 0:
                    get_ea[0].ReListData()
    ...
    def onSubData(self, _data):        
        ...
        if len(_topic)==self.sb_len:
            ...
            get_ea = list(filter(lambda x: x.Name == _topic[:self.sb_len], self.ea))
            #print("Update Price Rate : "+_time)
            if len(get_ea) > 0:
                price=[float(_close),float(_open),float(_high),float(_low),_time]
                if int(_topic[self.sb_len:])==get_ea[0].obj_input.timeframe:
                    Thread(target=get_ea[0].CalDiffTP(price)).start()
                    Thread(target=get_ea[0].SetFlag(price)).start()
                if int(_topic[self.sb_len:])==get_ea[0].obj_input.i_timeframe:
                    Thread(target=get_ea[0].SetTrend(price)).start()
    ...
    def run(self):        
        ...
        for s in self._symbols:
            self._instruments.append((s,240))
            self._instruments.append((s,1440))
        
        i=1
        for s in self._symbols:
            sb_obj=symbol_obj(s)
            self.sb.append(sb_obj)
            self.ea.append(EA_DualGrid(sb_obj, self,self.obj_input,i))
            i+=1
